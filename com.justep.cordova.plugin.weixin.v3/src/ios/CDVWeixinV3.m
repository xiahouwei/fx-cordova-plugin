...
- (void)LaunchMini:(CDVInvokedUrlCommand *)command{
    [self prepareForExec:command];
    WXLaunchMiniProgramReq *req = [WXLaunchMiniProgramReq object];
    NSDictionary *params = [self checkArgs:command];
    
    req.userName = [params objectForKey:@"userName"];  //拉起的小程序的username
    
    req.path = [params objectForKey:@"path"];   //拉起小程序页面的可带参路径，不填默认拉起小程序首页

    NSString *miniType = [params objectForKey:@"type"]; //拉起小程序的类型（正式0，开发1，体验2）addby shw 2019-10-09
    //小程序版本类型（正式0，开发1，体验2）
    if ([miniType isEqualToString: @"2"]) {
      req.miniProgramType= 2;
    } else if ([miniType isEqualToString: @"1"]) {
      req.miniProgramType= 1;
    } else {
      req.miniProgramType= 0;
    }
    // req.miniProgramType= 0; //拉起小程序的类型
...
- (void)share:(CDVInvokedUrlCommand *)command{
  ...
  if (message){
    req.bText = NO;
    if([[message objectForKey:@"title"] length] > 512){
      CDVPluginResult *result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"title长度不得超过512字节"];
      [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
      return;
    };
    
    if([[message objectForKey:@"description"] length] > 1024){
      CDVPluginResult *result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"description长度不得超过1024字节"];
      [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
      return;
    };
    
    if([[message objectForKey:@"mediaTagName"] length] > 64){
      CDVPluginResult *result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"mediaTagName长度不得超过64字节"];
      [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
      return;
    };
    
    if([message objectForKey:@"media"] == nil){
      CDVPluginResult *result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"media不能为空"];
      [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
      return;
    };
    ...
    [self.commandDelegate runInBackground:^{
      ...
      if(result == NO){
        CDVPluginResult *result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"分享失败"];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
        return;
      }
    }];
...
-(UIImage *) scaleImage:(UIImage *)image toKb:(NSInteger)kb{
  ...
  double targetwidth = sqrt(kb * 1000);
  if(image.size.width > targetwidth || image.size.height > targetwidth) {
    //计算宽高缩放率
    double compression = MIN(targetwidth / image.size.width, targetwidth / image.size.height);
    CGSize newsize;
    newsize.width = compression * image.size.width;
    newsize.height = compression * image.size.height;
...
- (UIImage *)getUIImageFromURL:(NSString *)thumb
{
  ...
  return [self scaleImage:[UIImage imageWithData:data] toKb:31];
}
